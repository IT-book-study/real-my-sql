...
#### 11.7.2.5 데이터베이스 삭제
- `DROP DATABASE`
- `IF EXISTS` 사용시 데이터베이스 존재시에만 삭제 

### 11.7.3 테이블 스페이스 변경
- 제너럴 테이블스페이스는 여러 테이블의 데이터를 한꺼번에 저장하는 테이블 스페이스를 의미
- 제너럴 테이블 스페이스의 제약사항
  - 파티션 테이블은 제너럴 테이블 스페이스 사용X
  - 복제 소스와 레플리카 서버가 동일 호스트 실행 시 ADD DATAFILE 문장 사용X
  - 테이블 암호화는 테이블 스페이스 단위로 설정됨
  - 테이블 압축 가능 여부는 테이블 스페이스의 블록, InnoDB페이지 사이즈로 결정
  - 특정 테이블을 삭제해도 디스크 공간이 운영체제로 반납되지 않음
- 제너럴 테이블 스페이스 장점
  - 파일 핸들러를 최소화
  - 테이블스페이스 관리에 필요한 메모리 공간 최소화
- 제너럴 테이블 스페이스는 테이블의 개수가 많은 경우 유용하게 사용 가능

### 11.7.4 테이블 변경
#### 11.7.4.1 테이블 생성
- `CREATE TABLE` 로 시작
- `칼럼명 + 칼럼타입 + [타입별옵션] + [NULL여부] + [기본값]` 순서로 명시
- 참고사항
  - 문자열 타입은 최대 저장 문자 수를 명시해야함
  - 숫자 타입은 크게 의미 없음, 양수만 저장하거나 양수 음수 둘다 저장하거나 선택 가능
  - ZEROFILL 키워드로 숫자값의 왼쪽 채우기 가능
  - ENUM, SET 타입은 이름 뒤 해당 칼럼이 가질 수 있는 값을 괄호로 명시해야함
#### 11.7.4.2 테이블 구조 조회
- `SHOW CREATE TABLE` 명령
  - 만들어진 테이블을 CREATE TABLE 명령으로 재작성해 보여줌
  - 칼럼의 목록, 인덱스, 외래키 정보 동시에 확인 가능
- `DESC` 명령
...
#### 11.7.4.4 테이블 명 변경
- `RENAME TABLE` 명령어 사용
- 다른 데이터베이스로 테이블을 이동할대도 사용
  - 서로 다른 운영체제의 시스템 사용시 데이터 파일 복사 작업이 필요해 시간이 소요된다.
- 테이블 교체 시 문제 발생을 없애기 위해 RENAME 명령어 하나에 여러 테이블명을 변경할 수 있다.
#### 11.7.3.5 테이블 상태 조회
- 테이블이 만들어진 시간, 대략의 레코드 건수, 데이터 파일 크기, 데이터파일 버전, 레코드 포맷 등의 정보를 조회할 수 있다.
- `SHOW TABLE STATUS ...`로 조회 가능, `LIKE` 명령어 사용으로 특정 테이블의 상태 조회 가능
#### 11.7.4.6 테이블 구조 복사
- `CREATE TABLE ... AS SELECT ... LIMIT 0` 으로 테이블 복사가 가능
#### 11.7.4.7 테이블 삭제
- `DROP TABLE` 명령어로 삭제
- 용량이 큰 테이블은 디스크에서 읽을때 부하가 많이 들어가 서비스 도중 삭제 작업은 수행하지 않는게 좋다.
- 어댑티브 해시 인덱스가 활성화 된 경우 어댑티브 해시 인덱스 정보도 함께 삭제 되어야하기 때문에 부하가 발생할 수 있다.
